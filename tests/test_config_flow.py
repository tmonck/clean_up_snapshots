...
async def test_async_step_user_complete_flow(hass: HomeAssistant):
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"], user_input={CONF_ATTR_NAME: num_backups_to_keep}
    )
    assert result["title"] == "clean_up_snapshots_service"
    assert result["type"] == FlowResultType.CREATE_ENTRY
    assert "data" in result
    assert result["data"][CONF_ATTR_NAME] == num_backups_to_keep
...
async def test_async_step_import_creates_entry(hass: HomeAssistant):
    ...
    result = await hass.config_entries.flow.async_init(
        ...
        data={CONF_ATTR_NAME: num_backups_to_keep},
    )
    assert result is not None
    assert result["type"] == FlowResultType.CREATE_ENTRY
    assert "data" in result
...
async def test_optionsflow(
    ...
    num_backups_to_keep = 3
    updated_num_backups_to_keep = 3
    entry_data = {CONF_ATTR_NAME: num_backups_to_keep}
    ...
    update_data = {CONF_ATTR_NAME: updated_num_backups_to_keep}
